...
// Generates a human-readable booking reference, e.g. BK-2026-0042
// Uses a counter from a shared Firestore counter doc for monotonic IDs.
export function generateBookingRef() {
  const year = new Date().getFullYear();
  // Use timestamp-based 4-digit suffix (last 4 chars of ms since epoch base-36)
...
export async function addBooking(bookingData) {
  ...
  console.log(`[addBooking] Created booking ${docRef.id} ref ${refNumber}`);
  return docRef.id;
}
...
/** Best-effort read of a user's notification preferences (fails open: returns null on error). */
export async function getUserNotificationPrefs(uid) {
  if (!uid) return null;
  try {
    const snap = await getDoc(doc(db, 'users', uid));
    return { ...DEFAULT_NOTIFICATION_PREFS, ...(snap.data()?.notificationPrefs || {}) };
  } catch (err) {
    console.warn('[getUserNotificationPrefs]', err.message);
    return null;
  }
}

/** True when prefs are unavailable (fails open) or the given key is explicitly not disabled. */
export function isPrefEnabled(prefs, key) {
  return prefs == null || prefs[key] !== false;
}

// ======================================================
// REAL-TIME LISTENERS  (use these instead of getDocs)
// Each function returns an unsubscribe function.
// Usage:  const unsub = subscribeToBookings(userId, setData)
//         useEffect(() => unsub, [])  // cleanup on unmount
// ======================================================

/** Real-time: user's own bookings */
export function subscribeToBookings(userId, callback) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToBookings]', err.message));
}

/** Real-time: all bookings (admin) */
export function subscribeToAllBookings(callback) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToAllBookings]', err.message));
}

/** Real-time: recent bookings for user dashboard */
export function subscribeToRecentBookings(userId, count = 5, callback) {
  if (!userId) { callback([]); return () => { }; }
  const q = query(collection(db, 'bookings'), where('userId', '==', userId));
  return onSnapshot(q, (snapshot) => {
    const results = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
  ...
  }, (err) => console.error('[subscribeToRecentBookings]', err.message));
}
...
export function subscribeToAdminNotifications(callback) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToAdminNotifications]', err.message));
}

/** Real-time: fleet types */
export function subscribeToFleetTypes(callback) {
  return onSnapshot(collection(db, 'fleetTypes'), (snapshot) => {
    callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
  }, (err) => console.error('[subscribeToFleetTypes]', err.message));
}

/** Real-time: trucks for sale */
export function subscribeToTrucksForSale(callback) {
  return onSnapshot(collection(db, 'trucksForSale'), (snapshot) => {
    callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
  }, (err) => console.error('[subscribeToTrucksForSale]', err.message));
}

/** Real-time: appointments */
export function subscribeToAppointments(userId, callback) {
  let q;
  if (userId) {
    q = query(collection(db, 'appointments'), where('userId', '==', userId));
  } else {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToAppointments]', err.message));
}
...
export function subscribeToAllUsers(callback) {
  return onSnapshot(collection(db, 'users'), (snapshot) => {
    callback(snapshot.docs.map((d) => ({ id: d.id, ...d.data() })));
  }, (err) => console.error('[subscribeToAllUsers]', err.message));
}

/** Real-time: transactions / purchase requests */
export function subscribeToPurchaseRequests(callback) {
  return onSnapshot(collection(db, 'purchaseRequests'), (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToPurchaseRequests]', err.message));
}

export function subscribeToUserPurchaseRequests(userId, callback) {
  if (!userId) return () => { };
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    callback(results);
  }, (err) => console.error('[subscribeToUserPurchaseRequests]', err.message));
}

export async function updatePurchaseRequest(requestId, data) {
