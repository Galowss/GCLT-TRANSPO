...
export function AuthProvider({ children }) {
  ...
  const buildUserObject = async (firebaseUser) => {
    ...
    return {
      ...
      notificationPrefs: { ...DEFAULT_NOTIFICATION_PREFS, ...(userData.notificationPrefs || {}) },
    };
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        setUser(u);
        document.cookie = "gclt_session=true; path=/; max-age=2592000; SameSite=Lax";
      } else {
        setUser(null);
        document.cookie = "gclt_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      }
      setLoading(false);
    });
  ...
  const refreshUser = async () => {
    if (auth.currentUser) {
      try {
        ...
        // A failed reload (e.g. transient network issue or revoked token) must
        // not wipe the signed-in state — keep the cached user until we know.
        console.warn('User reload failed, keeping cached session:', err.message);
        const cached = await buildUserObject(auth.currentUser);
        setUser(cached);
        return cached;
      }
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, register, loginWithGoogle, logout, resendVerification, refreshUser, resetPassword }}>
      {children}
